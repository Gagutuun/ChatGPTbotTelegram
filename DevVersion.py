...
bot = telebot.TeleBot("5920358297:AAH24s97cxlnx7WsUqA1tZ9RnHWS9lcmFpw")
...
def handle_start(message):
    ...
    itembtn3 = telebot.types.KeyboardButton('/showchats')
    itembtn4 = telebot.types.KeyboardButton('/save')
    itembtn5 = telebot.types.KeyboardButton('/load')
    itembtn6 = telebot.types.KeyboardButton('/changemodel')
...
def handle_message(message):
    ...
    # Show all extists chats
        ...
        chats = list(conversation_history.keys())
        if chats:
            reply_markup = telebot.types.InlineKeyboardMarkup()
            for chat in chats:
                reply_markup.add(telebot.types.InlineKeyboardButton(chat, callback_data=chat))
            bot.send_message(chat_id=chat_id, text = "Choose a chat to switch to:", reply_markup=reply_markup)
        else:
            bot.send_message(chat_id=chat_id, text = "No chats found.")
